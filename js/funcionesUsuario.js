...
$(document).tooltip().ready(function(){ 
    //tablas
    $("button").button(); 
   $('#listaUsuario').dataTable( { //CONVERTIMOS NUESTRO LISTADO DE LA FORMA DEL JQUERY.DATATABLES- PASAMOS EL ID DE LA TABLA
        "sPaginationType": "full_numbers",
        "aaSorting": [[ 4, "asc" ],[0,"asc"]],//DAMOS FORMATO A LA PAGINACION(NUMEROS)
        
    } );
            
    
    $("#listaUsuario_filter").find("input:first").attr("onkeyup","resalta()");
...
function resalta(){
     $("#listaUsuario").unhighlight();
     $(".highlight").css({ backgroundColor: "#FFFF88" });
    $("#listaUsuario").highlight($("#listaUsuario_filter").find("input:first").val());
}
